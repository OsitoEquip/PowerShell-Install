...
function global:Assert-Block([ScriptBlock]$Script, $Label, $Message, $Intent = $Intention.ShouldPass) 
{
    begin {
        ...
        {
            if (!(Test-Path variable:TestConditionFilter)) { $TestConditionFilter = $DefaultTestCaseFilter} 
...
function global:AssertEqual($Expected, $Actual, $Label, $Intent = $Intention.ShouldPass) 
{
    ...
    {
        $msg = $Expected.ToString() + $AssertEqualConjunction + "but was NULL."
        Assert-Block {$false} -Label $Label -Message $msg -Intent $Intent
    }
...
{
    if (!(Test-Path variable:RaiseAssertionFilter)) { $TestConditionFilter = $DefaultTestCaseFilter} 
    ...
    for ($a=0; $a -lt $Assertions.Count; $a++) 
    {
	    ...
	    if($ResultColour.Error -eq $foreColour){ $msg = $msg + " `a"}
